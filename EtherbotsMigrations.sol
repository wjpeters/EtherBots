...
contract EtherbotsMigrations is PerksRewards {
    ...
    //Part Id - (index into range of parts for part-type)
    ...
    // Element 
    string constant DEFENCE_ELEMENT_BY_ID = "12434133214";
    string constant MELEE_ELEMENT_BY_ID = "31323422111144";
    string constant BODY_ELEMENT_BY_ID = "212343114234111";
    string constant TURRET_ELEMENT_BY_ID = "43212113434";

    /*    struct Part {
        uint8 partType; //1, 2, 3, 4
        uint8 partId; //1-15
        uint8 rarity;
        uint8 element;
...
*/
    ...
    function _migrateRobot(string robot) internal {
    ...
    function _getRarity(string original, uint8 low, uint8 high) pure internal returns (uint8) {
    ...
    function _getElement(string elementString, uint partId) pure internal returns(uint8) {
        return stringToUint8(substring(elementString, partId-1,partId));
    }

    function _getPartId(string original, uint8 start, uint8 end, uint8 partCount) pure internal returns(uint8) {
        return (stringToUint8(substring(original,start,end)) % partCount) + 1;
    }

    function userPendingCrateNumber(address _user) view external returns (uint) {
        return pendingCrates[_user].length;
    }
    
    // event blueprints(uint8[4], uint8[4], uint8[4], uint8[4]) ;
    
    
    // convert old string representation of robot into 4 new ERC721 parts
  
    function _convertBlueprint(string original) pure internal returns(uint8[4] body,uint8[4] melee, uint8[4] turret, uint8[4] defence ) {
    ...
    function _migrateExpiredCrates() internal {
